...
# Required method to setup context.koduck. Can also be run as a comamnd after startup to update any manual changes to the commands table.
async def refresh_commands(context, *args, **kwargs):
    errors = []

    def cmd_func(row):
        if "advance" in row["Module"]:
            context.koduck.add_command(row['Command'], getattr(mainadvance, row['Function']), row['Type'], int(row['Permission']))
        elif "safety" in row["Module"]:
            context.koduck.add_command(row['Command'], getattr(mainsafety, row['Function']), row['Type'], int(row['Permission']))
        elif "roll" in row["Module"]:
            context.koduck.add_command(row['Command'], getattr(mainroll, row['Function']), row['Type'], int(row['Permission']))
        elif "base" in row["Module"]:
            context.koduck.add_command(row['Command'], getattr(mainnb, row['Function']), row['Type'], int(row['Permission']))
        elif "fools" in row["Module"]:
            context.koduck.add_command(row['Command'], getattr(mainaprilfools, row['Function']), row['Type'], int(row['Permission']))
        else:
            context.koduck.add_command(row['Command'], globals()[row['Function']], row['Type'], int(row['Permission']))
        return
    ...
    if context.message is not None:
        if errors:
            await context.koduck.send_message(context.message,
...
async def goodnight(context, *args, **kwargs):
    return await context.koduck.client.logout()
...
async def oops(context, *args, **kwargs):
    ...
    try:
        await context.koduck.delete_message(THEmessage)
