    ...
    if parameters[2] != 0:
        ...
        for i in range(len(results)):
            if results[i] <= parameters[2]:
                ...
                successes += 1
        return await koduck.sendmessage(context["message"], sendcontent="{} rolled a {} ({} successes)".format(context["message"].author.mention, ", ".join([str(x) for x in results]) if len(results) > 1 else results[0], successes))
    else:
        return await koduck.sendmessage(context["message"], sendcontent="{} rolled a {}".format(context["message"].author.mention, ", ".join([str(x) for x in results]) if len(results) > 1 else results[0]))

async def tag(context, *args, **kwargs):
    if len(args) < 1:
        return await koduck.sendmessage(context["message"], sendcontent="Give me a BattleChip tag and I can pull up its info for you!")
    table = yadon.ReadTable("tagdata")
    case_insensitive =  {key.lower():key for key in table.keys()}
    try:
        name = case_insensitive[args[0].lower()]
    except KeyError:
        return await koduck.sendmessage(context["message"], sendcontent="I don't recognize that BattleChip tag!")
...
async def chip(context, *args, **kwargss):
    if len(args) < 1:
        return await koduck.sendmessage(context["message"], sendcontent="Give me the name of a BattleChip and I can pull up its info for you!")
    table = yadon.ReadTable("chipdata")
    case_insensitive =  {key.lower():key for key in table.keys()}
    try:
        name = case_insensitive[args[0].lower()]
    except KeyError:
        return await koduck.sendmessage(context["message"], sendcontent="I don't recognize that BattleChip!")
    values = table[name]
    ...
    if "[MEGA]" in values[2]:
      color = 0xff0000
    elif "[ChitChat]" in values[2]:
      color = 0xff8000
    elif "[RadicalSpin]" in values[2]:
      color = 0x3f5cff
    elif "[UndergroundBroadcast]" in values[2]:
      color = 0x73AB50
...
async def NCP(context, *args, **kwargs):
    ...
    if power in ["LockOn", "Volley", "BlindMode", "Splash", "Tracker", "Refresh", "Reconfig", "Analyze", "Foresight", "Extend", "MapEdit", "HotSwap", "Disruption", "Firewall", "NoClip"]:
        color = 0x81A7C6
    elif power in ["BreakCharge", "Followthrough", "Gutsy", "Shockwave", "Shatter", "Warp", "Afterimages", "Jump Jets", "Sneakrun", "Artful Dodger", "Regenerate", "Clear", "Kinetic Armor", "Reflect", "Hyper Armor"]:
        color = 0xDF8F8D
    elif power in ["Overwrite", "ModelEdit", "Playback", "Harmless", "Hypnotize", "Rally", "Bodyguard", "Vengeance", "Glare", "Duel", "Save", "Code Injection", "Shift", "Control", "Alt"]:
        color = 0xF8E580
    else:
        color = 0xffffff
    embed = discord.Embed(title="__{}__".format(name), color=color)
...
async def query(context, *args, **kwargs):
    if len(args) < 1:
        return await koduck.sendmessage(context["message"], sendcontent="This command can sort battlechips by Category, and single out Crossover Content chips!")
    ...
    for chipname, values in table.items():
        if args[0].lower() == values[0].lower():
            ...
            results.append(chipname)
    if not results:
        return await koduck.sendmessage(context["message"], sendcontent="I can't find any chips or NCPs in that Category, or from that Crossover title.")
    else:
        return await koduck.sendmessage(context["message"], sendcontent="*Battlechips/NCPs in the ''__{}__'' category, or from that specific Crossover...*\n**{}**".format(args[0],", ".join(results)))
...
async def rulebook(context, *args, **kwargs):
    if len(args) < 1:
        return await koduck.sendmessage(context["message"], sendcontent="NetBattlers Beta 3 Official Rulebook (high-res): <https://drive.google.com/file/d/1s_Q1zuALa6WMWSvIvFkeqbOVLPvfephD/view>\nNetBattlers Beta 3 Official Rulebook (mobile-friendly): <https://drive.google.com/open?id=1zgrLwIVpli1QPsGOeUXIn13GnebRg0hR>\nNetBattlers Advance, The Supplementary Rulebook: <https://drive.google.com/file/d/1E--RQeBFuz1qM2bCdt125L3ymGa_wVAI/view?usp=sharing>")
