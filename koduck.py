...
class Koduck:
    ...
    async def send_message(self, receive_message: Optional[discord.Message] = None,
                           channel: Optional[discord.abc.Messageable] = None, ignore_cd: bool = False, **kwargs):
        ...
        if receive_message is not None:
            ...
            # removing cooldown for progbot, so that progbot can send multiple messages in a row
    ...
    def add_run_slash_command(self):
        async def run_command(interaction, command: str):
            ...
            message_content = settings.commandprefix + command
...
async def on_message(message):
    ...
    try:
        ...
        if message.content.startswith(serv_prefix):
            ...
            context.command_line = message.content[len(serv_prefix):]
            try:
        ...
        command_permission = koduck_instance.commands[context.command][2]

        if command_permission == USERLEVEL_USER:
            pass
        elif command_permission == USERLEVEL_SERVERADMIN:
            if context["message"].author.id != context["message"].guild.owner_id:
                await koduck_instance.send_message(message, sendcontent=settings.message_restrictedaccess)
                #log(None, settings.message_restrictedaccess)
                return
            pass
        elif user_level < command_permission:
